...
const TARGET_DATE = new Date('2026-03-11T00:00:00+02:00'); // SAST

function checkDateAndHideCountdown() {
    const now = new Date();
    if (now >= TARGET_DATE) {
        const overlay = document.getElementById('countdownOverlay');
        if (overlay) overlay.remove(); // completely removed from DOM
        return true;
    }
...
let dateHasPassed = checkDateAndHideCountdown();
if (!dateHasPassed) {
    const interval = setInterval(() => {
        if (checkDateAndHideCountdown()) {
    ...
    if (wish) {
        ...
        lineRevealTimers.push(setTimeout(() => {
            ...
            lines.forEach((line, index) => {
                const timer = setTimeout(() => line.classList.add('visible'), index * lineSpacing);
...
function endCelebration() {
    ...
    const wish = document.getElementById('wishMessage');
    if (wish) {
        wish.querySelectorAll('span').forEach(line => line.classList.add('visible'));
        wish.classList.add('breathe');
    }
...
function getRandomColor() {
    const colors = [
        '#ff4d94', '#ff8fab', '#ffb3d1', '#e6c3ff', '#ff6ec4',
        '#ff99cc', '#f8a5c2', '#c78cff', '#ff80ab', '#db7093',
        '#ffd700', '#00ff00'
    ];
